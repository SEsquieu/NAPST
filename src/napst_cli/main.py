...
app = typer.Typer(help="NAPST - behavioral probing and boundary inference for agentic systems")
console = Console()
...
@app.command()
def run(target_file: str, scenario: str = typer.Option("boundary_baseline", help="Scenario pack to execute")) -> None:
    """Run a scenario against a target YAML config."""
    target = TargetConfig.from_yaml(target_file)
    runner = NapstRunner()
    result = runner.run(target, scenario)
    console.print(f"[green]Completed[/green] {result.run_id} against [bold]{result.target_name}[/bold] with scenario [bold]{result.scenario}[/bold]")
...
def list_runs() -> None:
    ...
    for col in ("Run ID", "Target", "Scenario", "Created", "Findings"):
        table.add_column(col)
    for row in rows:
        table.add_row(row["run_id"], row["target_name"], row["scenario"], row["created_at"], str(row["finding_count"]))
    console.print(table)
